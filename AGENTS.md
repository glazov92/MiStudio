...
- **Форма записи = САЙТОВАЯ** (v2.2): «Записаться»/карточка акции открывают BottomSheet на ~90% экрана с WebView сайта; BOOKING_JS прячет секции и сам открывает сайтовой попап «Запись на визит» (`openPopup(#lead-popup)`); клик по `[data-close-popup]` → `AppBridge.closeLead()` закрывает шторку. Заявки идут штатным submitLead сайта → VK-бот + email. Кастомная нативная форма удалена.
