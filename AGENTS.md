...
- **Кнопка звонка (v2.2)**: круглая 64dp ВНУТРИ таб-бара по центру (оверлеи `pulse`+`call_wrap` гравитацией bottom|center, translationY=(H−navH)/2 — центрирует ЛЮБУЮ вьюху на середину бара). PulseView: 3 кольца при старте и раз в 2 мин. Табы 4: Главная · Услуги · Работы · Контакты.
...
- Версии: versionCode/versionName в `android/app/build.gradle`. История: v1.0(1) обёртка; v1.1(2) табы-скролл; v1.1.1(3) без PTR+сайтная форма; v1.1.2(4) фикс spy; v2.0(5) раздельные экраны; v2.1(6) всё нативное+загрузчик; **v2.2(7) — звонок внутри таб-бара, фото на главной, мастера сверху, сайтова форма заявки**.
