...
- Кнопка «Записаться» (v1.1.1) открывает САЙТОВУЮ форму «Запись на визит» (`openPopup(#lead-popup)` — глобальная функция main.js, скрипты классические): выбор услуг галочками, способ связи, валидация; заявка уходит через сайтный submitLead → беседа VK + email. Фолбэк при ошибке — скролл к #contacts.
- Скролл к секциям (v1.1.1) — мгновенный `scrollIntoView(true)` c задержкой 60 мс (сайт использует OverlayScrollbars на body — smooth-анимации и нативный window.scrollY там ненадёжны). Scroll-spy — IntersectionObserver по секциям + сравнение getBoundingClientRect с 0.4×innerHeight, результат в `AppBridge.section(id)` → подсветка таба.
- ВАЖНО (v1.1.2): программный `nav.setSelectedItemId()` вызывает OnItemSelectedListener → без флага `suppressNavSelection` spy-подсветка запускала встречный скролл = «дёрганье». Порядок табов = физический порядок страницы: Главная · Акции · Услуги · Работы · Контакты (иначе подсветка прыгает немонотонно). Spy троттлится 150 мс и блокируется 700 мс после программного перехода (`__appLockUntil`).
- Pull-to-refresh УДАЛЁН в v1.1.1: SwipeRefreshLayout конфликтовал с кастомным скроллом сайта (ложные срабатывания refresh при жестах). Обновление страницы = перезапуск приложения / кнопка «Повторить» на оффлайн-экране.
- Данные для приложения (телефон, вебхук, ссылки) сайт пушит при загрузке через `AppBridge.config` — правки из админки подхватываются без пересборки APK. В config.js есть пустое поле `bookingUrl` под YClients (сайт его игнорирует).
...
- Версии: `versionCode`/`versionName` в `android/app/build.gradle`; перед релизом инкрементить versionCode. История: v1.0 (1) — обёртка; v1.1 (2) — табы, маскировка; v1.1.1 (3) — без PTR, мгновенные якоря, запись через сайтный попап; v1.1.2 (4) — порядок табов по странице, фикс spy-петли (suppressNavSelection), троттлинг.
