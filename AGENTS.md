...
- **Модель публикации (24.08.2026, вечер):** локальные сохранения (модалки/менеджеры) НЕ отправляют ничего на сервер — только localStorage + флаг `editorDirty` (событие mi-dirty, красная точка на кнопке 💾). Публикация — только явное нажатие «💾 Сохранить» на панели: полный снимок + пересборка MI_SNAPSHOT + дифф в VK. Кнопки в модалках переименованы в «Применить». editorSyncPush имеет ретраи ×3 (1.2s*attempt) на 408/429/5xx и сетевых сбоях; 403 не ретраится; честные тосты вместо «сервер недоступен». beforeunload предупреждает при несохранённом.
...
- **Дизайн главной v2.5 (референс Fresha/премиум-wellness)**: hero переведён в тёмный (#211E19, bg_hero_dark) с золотой надстрочкой BEAUTY ATELIER (letterSpacing), белым логотипом, золотой линией-разделителем и декоративной ✦; баннер записи — светлая карточка (elevation 2) с иконкой 📅 в золотом круге и стрелкой ›; заголовок акций с золотым маркером-полоской (bg_accent_bar); bg_card радиус 18. Функционал без изменений (баннер/акции).
