...
- **MI_SNAPSHOT (24.08.2026):** после каждой мутации хранилища `refreshSnapshot()` атомарно (tmp+rename, flock `.index-lock`) обновляет блок `<!--MI_SNAPSHOT-->…<!--/MI_SNAPSHOT-->` в index.html — JSON текущих правок. Фронт: инлайн в head → `editorMergeSnapshot()` применяет его при загрузке, если synced_at снимка не старше LS-кэша (`<prefix>synced_at`). Публикация = «Сохранить» (деплой кода — отдельно). Пустые map-ключи в снимке кодируются `{}` (не `[]`).
