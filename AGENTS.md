...
## Android-приложение (v2.0 «эксперимент», 24.08.2026)
- Папка `android/`. **Сайт НЕ изменяется** — приложение читает данные со страницы через JS-мост (`AppBridge.config/services/promos` из CONFIG/getServices()/getPromos()), правки админки подхватываются живьём.
- Визуально РАЗДЕЛЬНЫЕ страницы (не скролл одного лендинга):
  - **Главная — нативная** (`page_home.xml`): золотая hero-карточка (Mi Studio / слоган / график+адрес из CONFIG), блок «О студии», лента АКЦИЙ (горизонтальные карточки badge/tag/title/desc/note из getPromos(); таба «Акции» больше нет — акции переехали сюда). Тап по карточке акции → форма записи.
  - **Услуги** (`#services`), **Работы** (`#portfolio`), **Контакты** (`#contacts`) — каждый свой WebView, грузится ЛЕНИВО при первом заходе, стартует мгновенным скроллом к своей секции (появление через 350 мс после onPageFinished, чтобы не мигало), состояние скролла сохраняется между переключениями. Сайтные попапы услуг/заявки внутри работают как есть.
- Таб-бар: Главная · Услуги · [📞 Позвонить] · Работы · Контакты — звонок в ЦЕНТРЕ: диалог выбора номера (номера из CONFIG.phones через мост) → tel:. Кнопка «Записаться» (глобальная, над табами) → нативная BottomSheet-форма (имя/телефон/услуга из Spinner/время/комментарий) → POST на CONFIG.leadWebhookUrl (`source:'android-app'`) → беседа VK + email; вторая кнопка — онлайн-запись (bookingUrl→dikidiUrl).
- Из v1.x сохранено: маскировка сайта (CSS прячется header/footer/#float-root/.mobile-cta), оффлайн-экран с «Повторить», file chooser админки, внешние ссылки наружу, yandex-карта внутри, DOM storage, back=history back активного WebView.
- Сборка: GitHub Actions `.github/workflows/android-apk.yml`; артефакт всегда, на теге `v*` — Release. Подпись: `android/signing/mistudio.p12` в приватном репо (пароль в app/build.gradle; не публиковать репо).
- Версии: versionCode/versionName в `android/app/build.gradle`. История: v1.0(1) обёртка; v1.1(2) табы-скролл+своя форма; v1.1.1(3) без PTR, якоря мгновенно, запись через сайтный попап; v1.1.2(4) порядок табов+фикс spy-петли; **v2.0(5) — раздельные экраны, нативная главная с акциями, звонок в центре таб-бара**.
- minSdk 24, targetSdk 35, applicationId `ru.studiomi.app`, material 1.12.0, Java 17. Иконки: adaptive + legacy PNG (Pillow).
