...
- **Даты в форме v2.3.2**: модель «ключ ISO → подпись» (TreeMap selDates, без обратного парсинга подписи — раньше из-за этого ✕ не удалял дату). Календарь после каждой выбранной даты открывается снова (старт с последней выбранной) — мультивыбор подряд; «Отмена»/«Назад» завершают.
- Версии: versionCode/versionName в `android/app/build.gradle`. История: v1.0(1) обёртка; v1.1(2) табы-скролл; v1.1.1(3) без PTR+сайтная форма; v1.1.2(4) фикс spy; v2.0(5) раздельные экраны; v2.1(6) всё нативное+загрузчик; v2.2(7) звонок в баре/фото/мастера сверху; v2.2.2(8) волны 20с+фикс шторки; v2.2.3(9) бары/волны 5с; v2.3(10) нативная форма заявки; v2.3.1(11) крестик формы/полное описание услуг/кнопки акций; **v2.3.2(12) — фикс удаления дат ✕ и мультивыбор календарём**.
