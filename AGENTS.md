...
- Защита сайта (16.08.2026): корневой `.htaccess` — HTTPS-редирект + HSTS (анти-SSL-стриппинг), X-Frame-Options/nosniff/Referrer-Policy, `Options -Indexes`, блок доступа к `.git`/точечным файлам (кроме `.well-known`); `data/.htaccess` и `data/versions/.htaccess` закрывают файлы правок; `content-sync.php` проверяет Origin (свой домен или localhost) и троттлит записи (1 POST/сек → 429). Полной серверной авторизации нет — это осознанно «дверь с защёлкой». Репозиторий приватный.
