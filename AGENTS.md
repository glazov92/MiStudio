...
- **Кнопка звонка**: круглая 68dp (radial gold bg_call_circle) над центром таб-бара (`call_wrap`, translationY=-nav.height/2, пересчёт при layout), позади PulseView — расходящиеся золотые кольца: вспышка из 3 колец сразу при старте и затем раз в 2 минуты (PERIOD_MS=120000). Клик → диалог выбора номера → tel:. Табов теперь 4: Главная · Услуги · Работы · Контакты.
...
- Версии: versionCode/versionName в `android/app/build.gradle`. История: v1.0(1) обёртка; v1.1(2) табы-скролл; v1.1.1(3) без PTR+сайтная форма; v1.1.2(4) фикс spy; v2.0(5) раздельные экраны+нативная главная+звонок в табах; **v2.1(6) — всё нативное, скрытый загрузчик данных, круглая кнопка с кольцами раз в 2 мин**.
